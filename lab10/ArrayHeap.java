...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        boolean sinkLeft = min(index, leftIndex(index)) == leftIndex(index);
        boolean sinkRight = min(index, rightIndex(index)) == rightIndex(index);

        while (sinkLeft || sinkRight) {
            int target = min(leftIndex(index), rightIndex(index));
            swap(index, target);
            index = target;
            sinkLeft = min(index, leftIndex(index)) == leftIndex(index);
            sinkRight = min(index, rightIndex(index)) == rightIndex(index);
        }
    }
    ...
    public void changePriority(T item, double priority) {
        // First find the index of the Node - Theta(N)
        int index = 1;
        while (!contents[index].item().equals(item)) {
            index++;
        }

        // Change priority and sink or swim
